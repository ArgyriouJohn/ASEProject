...
public class ProfileActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        byte [] encodeByte=Base64.decode(user.getPicture(),Base64.DEFAULT);
		...
		editProfileButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (firstName.isEnabled() == false && lastName.isEnabled() == false) {
					...
					Bitmap profilePic = pic.getDrawingCache();
//					int bytes = profilePic.getWidth()*profilePic.getHeight()*4;
//					ByteBuffer buffer = ByteBuffer.allocate(bytes); //Create a new buffer
//					profilePic.copyPixelsToBuffer(buffer); //Move the byte data to the buffer
...
//					System.out.println(strvalue + firstName.getText().toString() + lastName.getText().toString() + "Male" + myDay + myMonth + myYear);
//					System.out.println(entry.updateUserInfo(strvalue, firstName.getText().toString(), lastName.getText().toString(), radioButtonValue, myYear, myMonth+1, myDay));
					entry.close();
				}
			}
		});
